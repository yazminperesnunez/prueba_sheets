    ...
    card.innerHTML = `
      <div class="card p-3 h-100 quote-card ${esSel ? 'selected' : ''}" onclick="p2pSeleccionarProveedor('${c.id}')">
        ...
        <div class="mt-auto pt-2 border-top text-center">
          <button type="button" class="btn btn-sm ${esSel ? 'btn-primary' : 'btn-outline-primary'} w-100 fw-semibold">
            ${esSel ? '✓ Proveedor Seleccionado' : 'Seleccionar este Proveedor'}
          </button>
        </div>
      </div>
    `;
      ...
      ${cots.map(c => `
        <td>
          <button type="button" class="btn btn-sm ${estadoP2P.proveedorSeleccionado && estadoP2P.proveedorSeleccionado.id === c.id ? 'btn-success' : 'btn-outline-primary'}" onclick="p2pSeleccionarProveedor('${c.id}')">
            ${estadoP2P.proveedorSeleccionado && estadoP2P.proveedorSeleccionado.id === c.id ? '✓ Ganador Adjudicado' : 'Adjudicar'}
          </button>
        </td>
      `).join('')}
    </tr>
